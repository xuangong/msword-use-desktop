...
namespace MswordUse.WordDriver.Roslyn
{
    ...
    public static class Host
    {
        ...
        public static ExecResult Run(string code)
        {
            ...
            // Lazy attach: scripts that only use System types or Print() don't
            // need Word. Avoid touching COM in that path; on machines without
            // Word open, ROT probing can take seconds and makes driver smoke
            // tests flaky.
            Word.Document doc = null;
            Word.Application app = null;
            IDictionary<string, Word.Document> refs = new Dictionary<string, Word.Document>(StringComparer.OrdinalIgnoreCase);
            if (RequiresWordSession(code))
            {
                try
                {
            ...
            var globals = new Globals
            {
                Doc = doc,
                App = app,
                Refs = new Dictionary<string, Word.Document>(refs, StringComparer.OrdinalIgnoreCase),
        ...
        static bool RequiresWordSession(string code)
        {
            return code.IndexOf("Doc", StringComparison.Ordinal) >= 0
                || code.IndexOf("App", StringComparison.Ordinal) >= 0
                || code.IndexOf("Track", StringComparison.Ordinal) >= 0
                || code.IndexOf("Refs", StringComparison.Ordinal) >= 0;
        }
    }
}
